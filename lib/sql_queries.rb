...
"select title, amount - total from projects a left join (select a.id, sum(amount) as total from projects a left join pledges b on a.id = b.project_id group by title) b on a.id = b.id where a.amount >= b.total"
end
