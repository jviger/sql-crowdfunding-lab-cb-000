...
"select b.name, a.total from (select user_id, sum(amount) as total from pledges group by user_id) a left join users b on a.user_id = b.id group by name order by amount, name"
