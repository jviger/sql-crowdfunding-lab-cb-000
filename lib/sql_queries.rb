...
"select b.category, total from (select project_id, sum(amount) as total from pledges group by project_id) a left join projects b on a.project_id = b.project_id where b.category = 'music'"
end
